...
class SprintGraph:
    ...
    def __add_nodes(self):
        ...
        for issue in backlog:
            ...
            self.__graph.add_node(issue[SprintGraph.__NODE_ATTR_KEY],
                                  attr_dict={SprintGraph.__NODE_ATTR_TYPE: issue[ParamConstants.PARAM_ITEM_KEY],
                                             SprintGraph.__NODE_ATTR_DUEDATE: issue[SprintGraph.__NODE_ATTR_DUEDATE],
                                             SprintGraph.__NODE_ATTR_STARTDATE: start_date,
                                             SprintGraph.__NODE_ATTR_ENDDATE: end_date,
                                             SprintGraph.__NODE_ATTR_ESTIMATE: issue[SprintGraph.__NODE_ATTR_ESTIMATE],
                                             SprintGraph.__NODE_ATTR_WHRS: whrs,
                                             SprintGraph.__NODE_ATTR_EXT: False})

    def __add_ext_node(self, node):
        if node not in self.__graph.nodes:
            self.__graph.add_node(node, attr_dict={SprintGraph.__NODE_ATTR_EXT: True})
    ...
    def __add_edges(self):
        links = Accessor.factory(DbConstants.CFG_DB_SCRUM_API).get(
            {AccessParams.KEY_COLLECTION: DbConstants.SCRUM_BACKLOG_LINKS,
             AccessParams.KEY_TYPE: AccessParams.TYPE_MULTI})
        for link in links:
            link_type = link[SprintGraph.__EDGE_ATTR_TYPE]
            ...
            if link_type in [SprintGraph.__EDGE_TYPE_SUBTASK, SprintGraph.__EDGE_TYPE_BLOCKS]:
                self.__graph.add_edge(link_a, link_b, attr_dict={SprintGraph.__EDGE_ATTR_TYPE: link_type})
            elif link_type == SprintGraph.__EDGE_TYPE_BLOCKED and (link_b, link_a) not in self.__graph.edges:
                self.__graph.add_edge(link_b, link_a, attr_dict={SprintGraph.__EDGE_ATTR_TYPE: SprintGraph.__EDGE_TYPE_BLOCKS})
