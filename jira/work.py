...
class SprintBacklogRequest(Request):
    CFG_SPRINT_BACKLOG_REQUEST = './cfg/sprint-backlog-request.json'
    KEY_BACKLOG_ITEMS = 'issues'

    def __init__(self, login, pswd):
        with open(SprintBacklogRequest.CFG_SPRINT_BACKLOG_REQUEST) as cfg_file:
    ...
    def _parse_response(self, response, out_data):
        backlog = {}
        self.__parse_field(response, self._response_cfg[SprintBacklogRequest.KEY_BACKLOG_ITEMS], backlog)
        for issue in backlog[SprintBacklogRequest.KEY_BACKLOG_ITEMS]:
