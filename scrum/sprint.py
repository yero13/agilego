...
class Transformation:
    ...
    def transform_object(cfg, obj):
        for transform_item in cfg:
            ...
            # ToDo: object transform should be common for objects and datasets
            exec(transform_func) # ToDo: change to compile/eval (need to add key as target in cfg)
        return obj


# ToDo: this class should be generalized for single object transform
class Sprint:
...
# ToDo: this class should be generalized for array(dataset) of objects transform
class Wbs:
    """
    Represents sprint backlog tasks. Provides functionality for checking sprint backlog consistency
    """
    # ToDo: move to cfg file
    __CFG_SPRINT_BACKLOG_BREAKDOWN = './cfg/scrum/sprint-backlog-breakdown.json'

    __CFG_KEY_SCOPE_DATASET = 'dataset'
    __CFG_KEY_SCOPE_MODIFY_FIELDS = 'modify-fields' # ToDo: add fields and transform sign eval/lambda
    ...
    def __init__(self, work_records):
        self.__logger = logging.getLogger(__class__.__name__)
        self.__logger.info('sprint backlog input: {:d} work items'.format(len(work_records)))
        self.__work_df = pd.DataFrame.from_records(work_records)
    ...
    def __create_subset(self, index, join_on, columns, order, sort):
        ...
        # ToDo: other join options + check performance
        subset_df = index_df.set_index(join_on, drop=False).join(self.__work_df[columns].set_index(join_on, drop=False),
                                                                 on=[join_on], lsuffix='_left')
    ...
    @classmethod
    def from_records(cls, work_records):
        """
        Construct SprintBacklog from dictionary of work items

        :param work_records: {work item key: work item descriptor}
        :return: SprintBacklog
        """
        return cls(work_records)
